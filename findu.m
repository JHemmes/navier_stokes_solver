function [out] = findu(i_in,j_in, N, u, pres_col)

    % returns u_{i,j} for i_in, j_in. i_in and j_in can be lists
    ...
    if length(u) == 2*N*(N+1)
        ...
        for kk = 1:length(i_in)
            ...
            if ismember(index, pres_col)
                out(kk) = 0; % note that this should actually come from u_norm. This way the normal velocity BC are hardcoded to be 0
            else 
                idx_list = 1:2*N*(N+1);
                idx_list(pres_col) = [];
