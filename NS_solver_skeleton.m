...
% expression to determine delta t
a =    0.001747;
b =  -2.441e-05;
c =        1.13;
d =      -0.1854;
...
tE21_norm = tE21(:, col_idx_tE21);                                           % orientation of the normal velocities
...
u_norm = tE21_norm*u_norm_bc';                                               % multiply direction and magnitude to get u norm vector 
...
% make tsigma vector containing the primal grid lengths where the fluxes act across
for ii = 1:N
...
% Set up the pressure matrix for the Poisson equation
...
%         diff = 1e-7 % used in debugging 
        ...
        figure(1)
        plot(iter/1000, diff, 'bx')
        hold on
        
        diff_list(end+1) = diff;
        toc
    end
...
% line below only used when plotting known solutions. 
% load('output/N64_workspace.mat')
...
contour(x(2:end-1),x(2:end-1), p_mat, bot.contour_p)
grid on 
axis equal
...
axis([0 1 0 1])
filename = ['figures/contour_' num2str(N) '_p'];
print(filename, '-depsc2')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% VORTICITY %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

txi_mat = flip(reshape(txi,N+1,N+1)',2); % vorticity on primal grid 0-nodes in matrix 
 
figure('Units','centimeters',...
    'Position',[0 0 width height],...
    'PaperPositionMode','auto');
contour(tx,tx, txi_mat, bot.contour_xi)
grid on 
axis equal
xlabel('$x$', 'Interpreter', 'LaTeX')
ylabel('$y$', 'Interpreter', 'LaTeX');
...
set(gca,'GridLineStyle','--')
yticks(xticks)
filename = ['figures/contour_' num2str(N) '_xi'];
print(filename, '-depsc2')
...
psi_mat = flip(flip([psi_mat;zeros(size(psi_mat(1,:)))],2));


figure('Units','centimeters',...
    'Position',[0 0 width height],...
    'PaperPositionMode','auto')
contour(tx,tx, psi_mat, bot.contour_psi)
grid on 
axis equal
xlabel('$x$', 'Interpreter', 'LaTeX')
ylabel('$y$', 'Interpreter', 'LaTeX')
...
set(gca,'GridLineStyle','--')
yticks(xticks)
filename = ['figures/contour_' num2str(N) '_sl'];
print(filename, '-depsc2')
...
v_d0_mat = -flip(reshape(v_d0,N,N)',2);
...
% save variables on centreline for plotting
save(['output/N' num2str(N)  '_centreline_var'], 'sol')

% save current workspace
save(['output/N' num2str(N)  '_workspace'])
...
figure()
plot(bot.constx_y, bot.constx_u)
hold on 
plot(sol.constx_y, sol.constx_u)


figure()
plot(bot.constx_y, bot.constx_p)
hold on 
plot(sol.constx_y, sol.constx_p)

figure()
plot(bot.constx_y, bot.constx_xi)
hold on 
plot(sol.constx_y_xi, sol.constx_xi)

%%%%%%%%%%%%%%%%%%%%% PLOT ALONG HORIZONTAL CENTRELINE %%%%%%%%%%%%%%%%%%%%

figure()
plot(bot.consty_x, bot.consty_v)
hold on 
plot(sol.consty_x, sol.consty_v)


figure()
plot(bot.consty_x, bot.consty_p)
hold on 
plot(sol.consty_x, sol.consty_p)

figure()
plot(bot.consty_x, bot.consty_xi)
hold on 
plot(sol.consty_x_xi, sol.consty_xi)
...
set(gca,'GridLineStyle','--')
axis([0 1 0 1])
filename = 'figures/contour_bot_p';
print(filename, '-depsc2')

figure('Units','centimeters',...
    'Position',[0 0 width height],...
    'PaperPositionMode','auto');
I = imread('figures\bot_xi.png'); 
image([0 1],[0 1],I); 
grid on 
axis equal
xlabel('$x$', 'Interpreter', 'LaTeX')
...
set(gca,'GridLineStyle','--')
axis([0 1 0 1])
filename = 'figures/contour_bot_xi';
print(filename, '-depsc2')

figure('Units','centimeters',...
    'Position',[0 0 width height],...
    'PaperPositionMode','auto');
I = imread('figures\bot_sl.png'); 
image([0 1],[0 1],I); 
grid on 
axis equal
xlabel('$x$', 'Interpreter', 'LaTeX')
...
set(gca,'GridLineStyle','--')
axis([0 1 0 1])
filename = 'figures/contour_bot_sl';
print(filename, '-depsc2')
